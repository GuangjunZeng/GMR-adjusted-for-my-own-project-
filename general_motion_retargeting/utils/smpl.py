...
def load_smpl_file(smpl_file, smpl_body_model_path):
    smpl_data = np.load(smpl_file, allow_pickle=True)
    
    # Convert SMPL format to SMPLX format for processing
    if "poses" in smpl_data:
        poses = smpl_data["poses"]
        if poses.shape[1] > 72:
            poses = poses[:, :72]
        
        # Map SMPL poses to SMPLX format
        root_orient = poses[:, :3]
        body_pose = poses[:, 3:66]  # 21 joints x 3 = 63
    else:
        # If already in SMPLX format (from conversion)
        root_orient = smpl_data["root_orient"]
        body_pose = smpl_data["pose_body"]
    
    # Handle betas - pad from 10 to 16 if needed
    betas = smpl_data["betas"]
    if len(betas.shape) == 1 and betas.shape[0] == 10:
        betas = np.concatenate([betas, np.zeros(6)])
    elif len(betas.shape) == 2 and betas.shape[1] == 10:
        betas = np.concatenate([betas, np.zeros((betas.shape[0], 6))], axis=1)
    
    body_model = smplx.create(
        smpl_body_model_path,
        "smplx",
        gender=str(smpl_data["gender"]) if "gender" in smpl_data else "neutral",
        use_pca=False,
    )
    # print(smpl_data["pose_body"].shape)
    # print(smpl_data["betas"].shape)
    # print(smpl_data["root_orient"].shape)
    # print(smpl_data["trans"].shape)
    ...
    smpl_output = body_model(
        ...
        transl=torch.tensor(smpl_data["trans"]).float(), # (N, 3)
    ...
    return smpl_data, body_model, smpl_output, human_height
...
def get_smpl_data(smpl_data, body_model, smpl_output, curr_frame):
    """
    ...
    """
    global_orient = smpl_output.global_orient[curr_frame].squeeze()
    full_body_pose = smpl_output.full_pose[curr_frame].reshape(-1, 3)
    joints = smpl_output.joints[curr_frame].detach().numpy().squeeze()
...
def get_smpl_data_offline_fast(smpl_data, body_model, smpl_output, tgt_fps=30):
    """
    Must return a dictionary with the following structure:
    {
        "Hips": (position, orientation),
    ...
    """
    # Handle different frame rate field names
    if "mocap_frame_rate" in smpl_data:
        src_fps = smpl_data["mocap_frame_rate"].item()
    elif "mocap_framerate" in smpl_data:
        src_fps = smpl_data["mocap_framerate"].item()
    else:
        src_fps = 30  # default
    ...
    global_orient = smpl_output.global_orient.squeeze()
    full_body_pose = smpl_output.full_pose.reshape(num_frames, -1, 3)
    joints = smpl_output.joints.detach().numpy().squeeze()
    joint_names = JOINT_NAMES[: len(body_model.parents)]
    parents = body_model.parents
    ...
    for curr_frame in range(len(global_orient)):
        ...
        smpl_data_frames.append(result)

    return smpl_data_frames, aligned_fps
