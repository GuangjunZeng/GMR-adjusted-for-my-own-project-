...
def process_single_frame(frame_data, frame_index, retarget, robot_motion_viewer, verbose=False):
    """
    Process a single frame using the retargeting system.
    This function is designed to be called in parallel.
    Consistent with smplx_to_robot.py - no error handling, let failures propagate.
    """
    # Extract frame data
    smplx_data = frame_data
    
    # Process the frame using retargeting - no error handling like smplx_to_robot.py
    ...
    return frame_index, qpos  # (index, result) - no error handling


def process_single_npz(smplx_file_path, output_path, robot, SMPLX_FOLDER, verbose=False, visualize=False, record_video=False, log_file=None, parallel_frames=200):
    """
    ...
    try:
        ...
        # Step 5: Process frames - parallel processing
        step_start = time.time()
        print(f"🔄 Step 5/8: Processing motion frames with {parallel_frames} parallel threads...")
        total_frames = len(smplx_data_frames)
        print(f"📊 Total frames to process: {total_frames}")
        
        # Pre-allocate results array to maintain frame order
        qpos_list = [None] * total_frames
        processed_count = 0
        
        # Process frames in parallel
        with ThreadPoolExecutor(max_workers=parallel_frames) as executor:
            # Submit all frame processing tasks
            future_to_index = {}
            for i, frame_data in enumerate(smplx_data_frames):
                future = executor.submit(process_single_frame, frame_data, i, retarget, robot_motion_viewer, verbose)
                future_to_index[future] = i
            
            # Collect results as they complete
            for future in as_completed(future_to_index):
                frame_index, qpos = future.result()  # No error handling - consistent with smplx_to_robot.py
                
                # Store result at correct position to maintain frame order
                qpos_list[frame_index] = qpos
                processed_count += 1
                
                # Show progress every 5% of frames
                if processed_count % max(1, total_frames // 20) == 0 or processed_count == total_frames:
                    ...
                    if processed_count > 0:
                        ...
                        print(f"   📈 Processed {processed_count}/{total_frames} frames ({progress_percent:.1f}%) - Avg: {avg_time_per_frame:.3f}s/frame, ETA: {estimated_remaining_time:.1f}s")
                    else:
                        print(f"   📈 Processed {processed_count}/{total_frames} frames ({progress_percent:.1f}%) - Starting...")
        
        # No error handling needed - consistent with smplx_to_robot.py behavior
        # If any frame fails, the exception will propagate and stop the entire process
        # This maintains the same behavior as the original smplx_to_robot.py
        ...
        print(f"   Step 5 - Process Frames:    {step_times['process_frames']:.2f}s (parallel: {parallel_frames} threads)")
        ...
        if log_file is not None:
            try:
                with open(log_file, 'a', encoding='utf-8') as f:
                    ...
                    f.write(f"  Step 5 - Process Frames:    {step_times['process_frames']:.2f}s (avg: {avg_time_per_frame:.3f}s/frame, parallel: {parallel_frames} threads)\n")
                    ...
                    if step_times:
                        ...
                        for step_name, step_time in step_times.items():
                            ...
                            if step_name == 'process_frames' and avg_time_per_frame > 0:
                                f.write(f"  {step_display_name}: {step_time:.2f}s (avg: {avg_time_per_frame:.3f}s/frame, parallel: {parallel_frames} threads)\n")
                            else:
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="Number of parallel threads for processing frames within a single file (default: 200).",
    )
    ...
    # Parallel frames processing info
    print(f"🚀 Frame parallel processing: {args.parallel_frames} threads per file")
    if args.parallel_frames > 100:
        print(f"⚠️  WARNING: Using {args.parallel_frames} parallel frame threads may cause GPU memory pressure!")
        print(f"   - Each thread processes one frame independently")
        print(f"   - High thread counts may lead to GPU memory competition")
        print(f"   - Consider reducing to 50-100 threads if experiencing memory issues")
        print()
    ...
    if args.num_threads == 1:
        ...
        for i, (index, npz_path) in enumerate(file_pairs):
            ...
            success, processing_time, frames_count = process_single_npz(npz_path, output_path, args.robot, SMPLX_FOLDER, args.verbose, args.visualize, args.record_video, str(log_file_path), args.parallel_frames)
